...
// Intentar crear directorio de forma segura solo si es posible
...
try {
  ...
  console.warn('⚠️ Could not create database directory (normal in Vercel):', err.message);
  dbDirCreated = false;
}
  ...
  if (!db) {
    try {
      ...
      if (!fs.existsSync(dbPath)) {
        console.warn('⚠️ Database file not found:', dbPath, '(normal in Vercel)');
...
// Promisify queries - THROW ERRORS when DB not available so fallback works
function runQuery(sql, params = []) {
  return new Promise((resolve, reject) => {
    const database = getDatabase();
    if (!database) {
      // Throw error so API endpoints know to use fallback
      return reject(new Error('Database not available - falling back to JSON'));
    }
...
const DAL = {
    async getFullQAData() {
      try {
        return await this._getFullQADataWithDB();
      } catch (err) {
        console.warn('⚠️ Error in getFullQAData, database may not be available:', err.message);
        // Retornar estructura mínima válida para que el frontend no rompa
        // El JSON fallback en qaDataLoader.js completará los datos
        return {
          summary: {
            totalBugs: 0,
            bugsClosed: 0,
          ...
          resolutionTimeData: {},
          metadata: { source: 'fallback', error: err.message }
        };
      }
    },
    async _getFullQADataWithDB() {
      // Resumen general
      let summary = {};
      try {
        summary = await DAL.getStatistics();
      } catch (err) {
        console.warn('⚠️ Could not load summary:', err.message);
        summary = { totalBugs: 0, bugsClosed: 0, testCasesTotal: 0, testCasesExecuted: 0 };
      }

      // Bugs por prioridad - convertir array a objeto indexado
      let bugsByPriorityArray = [];
      try {
        bugsByPriorityArray = await DAL.getBugsByPriority();
      } catch (err) {
        console.warn('⚠️ Could not load bugs by priority:', err.message);
      }
      ...
      let bugsByModule = [];
      try {
        bugsByModule = await DAL.getBugsByModule();
      } catch (err) {
        console.warn('⚠️ Could not load bugs by module:', err.message);
      }

      // Bugs por categoría
      let bugsByCategory = [];
      try {
        bugsByCategory = await DAL.getBugsByCategory();
      } catch (err) {
        console.warn('⚠️ Could not load bugs by category:', err.message);
      }

      // Datos de desarrolladores
      let developerData = [];
      try {
        developerData = await DAL.getDevelopersAnalysis();
      } catch (err) {
        console.warn('⚠️ Could not load developer data:', err.message);
      }
      
      // ========== TEST CASES METRICS (NEW) ==========
      // Obtener el valor correcto de test cases desde la vista
      let testCasesMetrics = null;
      try {
        testCasesMetrics = await DAL.getTestCasesMetrics();
      } catch (err) {
        console.warn('⚠️ Could not load test cases metrics:', err.message);
      }

      let testCasesDesigned = null;
      try {
        testCasesDesigned = await DAL.getTestCasesDesigned();
      } catch (err) {
        console.warn('⚠️ Could not load test cases designed:', err.message);
      }

      let testCasesBySprintRaw = [];
      try {
        testCasesBySprintRaw = await DAL.getTestCasesBySprint2024();
      } catch (err) {
        console.warn('⚠️ Could not load test cases by sprint:', err.message);
      }

      let testCasesExecutionRate = null;
      try {
        testCasesExecutionRate = await DAL.getTestCasesExecutionRate();
      } catch (err) {
        console.warn('⚠️ Could not load test cases execution rate:', err.message);
      }

      let testCasesReuseData = null;
      try {
        testCasesReuseData = await DAL.getTestCasesReuse();
      } catch (err) {
        console.warn('⚠️ Could not load test cases reuse data:', err.message);
      }
      
      // ========== BUG RESOLUTION STATUS (NEW) ==========
      // Obtener estado de resolución de bugs
      let bugResolutionRaw = [];
      try {
        bugResolutionRaw = await DAL.getBugResolutionStatus();
      } catch (err) {
        console.warn('⚠️ Could not load bug resolution status:', err.message);
      }
      ...
      try {
        executionSummary = await DAL.getExecutionSummary();
      } catch (err) {
        console.warn('⚠️ Could not load execution summary:', err.message);
        executionSummary = {
          total_executions: 0,
          passed: 0,
          failed: 0,
      ...
      let sprintDataRaw = [];
      try {
        sprintDataRaw = await DAL.getBugsBySprint();
      } catch (err) {
        console.warn('⚠️ Could not load bugs by sprint:', err.message);
      }
      ...
      let executedTestCasesRaw = [];
      try {
        executedTestCasesRaw = await DAL.getTestCasesBySprint();
      } catch (err) {
        console.warn('⚠️ Could not load executed test cases:', err.message);
      }

      let plannedTestCasesRaw = [];
      try {
        plannedTestCasesRaw = await DAL.getPlannedTestCasesBySprint();
      } catch (err) {
        console.warn('⚠️ Could not load planned test cases:', err.message);
      }
      const executedMap = {};
      ...
      // Testers / Reportado data - wrap in try-catch to handle DB unavailable
      let testerData = [];
      try {
        testerData = await getTestersSummaryWithProducts();
      } catch (err) {
        console.warn('⚠️ Could not load testerData from DB, will use JSON fallback:', err.message);
        testerData = [];
      }
  ...
  getFilterOptions: async function({ product = '', priority = '', status = '', testType = '', attribute = '', testLevel = '', ambiente = '' } = {}) {
    ...
    try {
      ...
      console.error('\u274C getFilterOptions: Database not available - error:', e.message);
      // IMPORTANT: Throw error so endpoints can use JSON fallback
      // DO NOT return empty arrays - that would hide the error
      throw new Error(`Database unavailable (getFilterOptions): ${e.message}`);
    }
  },
